...
SELECT
    ...
    COUNT(orderdetails.quantity) AS Number_of_Orders_Placed
FROM
	customers
JOIN
	orders ON orders.customer_id = customers.customer_id
...
SELECT
	...
	avg(orderdetails.quantity) AS Average_Quantity
FROM
	orderdetails
JOIN
...
SELECT
    ...
    RANK()
		OVER (
			ORDER BY
				products.price DESC) AS Price_Rank
FROM
	products
ORDER BY
	Price_Rank
...
SELECT
	customers.customer_name AS Customers,
    SUM(orders.total_amount) AS Total_Spending,
    RANK()
		OVER (
			ORDER BY
			sum(orders.total_amount) DESC) AS Highest_Spenders
FROM
	customers
JOIN
	orders ON orders.customer_id = customers.customer_id
...
SELECT 
    ...
    orders.total_amount AS Above_Average_Orders
FROM
	customers
JOIN
	orders ON orders.customer_id = customers.customer_id
...
SELECT 
    ...
    SUM(orderdetails.quantity * orderdetails.price) AS Total_Sales
FROM 
    Products
JOIN 
...
SELECT
	customers.customer_name AS Customers,
    orderdetails.quantity AS Number_Of_Orders
FROM
	customers
JOIN
	orders ON orders.customer_id = customers.customer_id
...
JOIN
	orders ON orders.customer_id = customers.customer_id
WHERE
	orders.order_date >= current_date() - INTERVAL '30 Ddays';
...
SELECT
	ROW_NUMBER()
		OVER (
			ORDER BY customers.customer_name) AS Row_Num,
	customers.customer_name AS Customer,
    products.product_name AS Product,
...
GROUP BY
    products.product_name
HAVING
    COUNT(orderdetails.order_id) < 10;


-- Q16 Product Popularity by Region:
...
SELECT
	customers.customer_city AS Region,
    COUNT(products.product_name) AS Product,
    SUM(orderdetails.quantity * orderdetails.price) AS Revenue
FROM
	customers
JOIN
	orders ON orders.customer_id = customers.customer_id
JOIN
	orderdetails ON orderdetails.order_id = orders.orders.order_id
JOIN
	products ON products.product_id = orderdetails.product_id
WHERE
	COUNT(products.product_name) >= 2
GROUP BY
	products.product_name
ORDER BY
	Revenue;
